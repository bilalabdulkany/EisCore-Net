    ...
    public class StaleEventCheckDbContext
    {
        ...
        public StaleEventCheck FindMIdAndEventType(int mId, string eventType)
        {
            using (var connection = new SqliteConnection(_databaseName))
            {
                try
                {
                    string sql = "SELECT ID, M_ID AS MID, EVENT_TYPE AS EVENTTYPE, EVENT_TIMESTAMP AS EVENTTIMESTAMP FROM STALE_EVENT_CHECK WHERE M_ID =@mId AND EVENT_TYPE=@eventType";


                    _log.LogDebug("Executing query: {sql} with variables [{a},{b}]", sql, mId, eventType);

                    StaleEventCheck staleEvent = connection.QueryFirst<StaleEventCheck>(sql, new { mId, eventType });
                    _log.LogInformation("ID from query: [ {a} ]", staleEvent);
                    return staleEvent;
                }
                catch (Exception e)
                {
        ...
        public int Save(StaleEventCheck staleEventCheck)
        {
            ...
            " SELECT CAST(@Id AS VARCHAR(255)), CAST (@mId AS VARCHAR(255)),CAST (@EventType AS VARCHAR(255)),  ,CAST (@EventTimestamp AS datetime))";
            ...
            using (var connection = new SqliteConnection(_databaseName))
            {
                try
                {
                    ...
                    _log.LogDebug("Executing query: {sqlite} with variables [{Id},{mId},{EventType},{EventTimestamp}]", sqlite, staleEventCheck.Id, staleEventCheck.mId, staleEventCheck.EventType, staleEventCheck.EventTimestamp);
